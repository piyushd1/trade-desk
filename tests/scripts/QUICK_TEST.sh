...
USER_ID="${USER_IDENTIFIER:?Error: USER_IDENTIFIER environment variable not set. Please set it before running this script.}"
...
NO_AUTH=$(curl -s "${BASE_URL}/data/zerodha/profile?user_identifier=$USER_ID")
...
PROFILE=$(curl -s -H "Authorization: Bearer $ACCESS_TOKEN" \
  "${BASE_URL}/data/zerodha/profile?user_identifier=$USER_ID")
...
MARGINS=$(curl -s -H "Authorization: Bearer $ACCESS_TOKEN" \
  "${BASE_URL}/data/zerodha/margins?user_identifier=$USER_ID")
...
POSITIONS=$(curl -s -H "Authorization: Bearer $ACCESS_TOKEN" \
  "${BASE_URL}/data/zerodha/positions?user_identifier=$USER_ID")
...
CAPABILITIES=$(curl -s "${BASE_URL}/data/zerodha/capabilities")
...
print_test "Step 8: Test Ownership Validation (admin cannot access user's session)"
...
if echo "$ADMIN_LOGIN" | grep -q '"status":"success"'; then
    ...
    # Try to access user's session as admin
    UNAUTHORIZED=$(curl -s -H "Authorization: Bearer $ADMIN_TOKEN" \
      "${BASE_URL}/data/zerodha/profile?user_identifier=$USER_ID")
    
    if echo "$UNAUTHORIZED" | grep -q '"code":403'; then
        print_pass "Ownership validation working - admin blocked from user's session"
    else
